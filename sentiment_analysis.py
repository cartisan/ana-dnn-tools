...
class SentimentClassifier:
    def __init__(self):
        self.sentiment_classifier = SentimentClassifier._create_sentiment_analysis_model()

    @staticmethod
    def _create_sentiment_analysis_model() -> Pipeline:
    ...
    def analyze_sentiment(self, text: str) -> str:
        ...
        """
        logging.debug(f"Start analyzing sentiment of {text=}")
        with TIMER:
            sentiment = self.sentiment_classifier(text)
        logging.debug(f"Finished inference in {TIMER.time:.3f}s")
...
def _evaluate_sentiment_models():
    ...
    r = SentimentClassifier()

    correct_labels = 0
    for sentence, true_label in labeled_sentences:
        label = r.analyze_sentiment(sentence)
...
def _infer_example_story():
    ...
    for fragment in story_fragments:
        label = r.analyze_sentiment(fragment)
